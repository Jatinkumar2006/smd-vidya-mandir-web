import { createContext, useContext, useState, useEffect } from 'react'
import api from '@/services/api'

const AuthContext = createContext(null)

export function AuthProvider({ children }) {
  const [user, setUser]       = useState(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const stored = localStorage.getItem('smd_user')
    if (stored) setUser(JSON.parse(stored))
    setLoading(false)
  }, [])

  const login = async (email, password) => {
    const { data } = await api.post('/auth/login', { email, password })
    if (data.requiresOtp) {
      return data // Return to component to handle OTP flow
    }
    localStorage.setItem('smd_token', data.token)
    localStorage.setItem('smd_user', JSON.stringify(data.user))
    setUser(data.user)
    return data.user
  }

  const verifyOtp = async (tempToken, otp) => {
    const { data } = await api.post('/auth/verify-otp', { tempToken, otp })
    localStorage.setItem('smd_token', data.token)
    localStorage.setItem('smd_user', JSON.stringify(data.user))
    setUser(data.user)
    return data.user
  }

  const logout = () => {
    localStorage.removeItem('smd_token')
    localStorage.removeItem('smd_user')
    setUser(null)
  }

  return (
    <AuthContext.Provider value={{ user, login, verifyOtp, logout, loading }}>
      {children}
    </AuthContext.Provider>
  )
}

export const useAuth = () => useContext(AuthContext)
