      ...
      >
        ...
        <div className="relative mb-3 w-full flex justify-center">
          <div className="relative w-20 h-20 rounded-full p-1 bg-gradient-to-tr from-smd-gold to-smd-blue shadow-lg">
            {result.photo_url && !result.photo_url.includes('unsplash') ? (
              <img src={result.photo_url} alt={result.student_name} className="w-full h-full rounded-full object-cover border-4 border-white bg-white" />
            ) : (
              <div className="w-full h-full rounded-full border-4 border-white bg-blue-50 text-smd-blue flex items-center justify-center font-bold text-2xl" style={{ fontFamily: "'Merriweather',serif" }}>
    ...
    <>
      ...
      <section className="py-20 bg-slate-50 min-h-[50vh]">
        <div className="container mx-auto px-6">
          {loading ? (
            <LoadingSpinner text="Loading hall of fame..." />
