  ...
  useEffect(() => {
    ...
    api.get('/documents').then(res => {
      ...
      const grouped = dbDocs.reduce((acc, doc) => {
        ...
        const fileLink = doc.file_url.startsWith('http') 
          ? `${import.meta.env.VITE_API_URL}/documents/view/${doc.id}` 
          : doc.file_url
          
        acc[category].push({ id: doc.id, title: doc.title, file: fileLink })
        return acc
      }, {})
      ...
      <section className="responsive-section-bottom">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={containerVariants} style={{ maxWidth: '1160px', margin: '0 auto' }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: '40px', textAlign: 'center', color: '#6b7280' }}>No documents found.</div>
          ) : (
            <motion.div initial="hidden" animate="visible" variants={containerVariants} className="responsive-grid-2">
