...
export default function AdminResults() {
  ...
  const openAdd = () => { setForm(EMPTY_FORM); setEditId(null); setSelectedFile(null); setShowModal(true) }
  const openEdit = (r) => {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: 12 }}>
        <h1 style={{ fontSize: '1.6rem', fontWeight: 700, color: C.navy }}>Top Results</h1>
        <button onClick={openAdd} style={{
          background: C.gold, color: C.navy, fontWeight: 700, fontSize: 14,
          padding: '10px 20px', borderRadius: 10, border: 'none', cursor: 'pointer',
          display: 'flex', alignItems: 'center', gap: 8,
        }}>
          <Plus size={16} /> Add Top Result
        </button>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ background: C.bg }}>
              {['Photo', 'Name', 'Year', 'Class', 'Score', 'Actions'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={r.id} style={{ borderTop: `1px solid ${C.border}`, background: i % 2 === 0 ? C.white : '#fafbff' }}>
                ...
                <td style={{ padding: '12px 16px', fontWeight: 600 }}>{r.year}</td>
