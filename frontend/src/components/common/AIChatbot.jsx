...
export default function AIChatbot() {
  ...
  const send = async () => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || 'Sorry, I am having trouble connecting to my brain right now. Please call +91-9001995272.';
  ...
  const renderChatButton = () => {
    if (open) {
      return (
        <button 
          onClick={() => { setOpen(false); setShowGreeting(false) }}
          className="w-14 h-14 bg-smd-blue rounded-full shadow-[0_6px_18px_rgba(0,0,0,0.25)] flex items-center justify-center cursor-pointer hover:bg-blue-900 transition-colors relative z-50 pointer-events-auto"
        >
    ...
    return (
      <button 
        ...
        className="relative w-[64px] h-[64px] cursor-pointer group z-50 pointer-events-auto border-none bg-transparent p-0"
      >
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-80 w-[calc(100vw-48px)] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden max-h-[70vh]">
          ...
          <div className="flex-1 p-3 space-y-3 overflow-y-auto min-h-[300px] text-sm bg-gray-50/50">
            {messages.map((m, i) => (
