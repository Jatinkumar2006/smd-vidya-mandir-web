...
You are allowed to use logical synonyms and MUST intelligently account for user spelling mistakes/typos (e.g., "secienecce" = "science", "cost" = "fees").
If the user asks an ambiguous question (like "12th class fees") and there are multiple options (e.g., Science vs Arts), you MUST list the fees for ALL available options or politely ask the user to specify which stream they mean.
If the user asks for information that is TRULY NOT explicitly mentioned in the context below (even after accounting for typos), you MUST reply with exactly this sentiment:
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const [noticesRes, careersRes, docsRes] = await Promise.all([
      ...
      pool.query('SELECT title, extracted_text FROM school_documents WHERE active = TRUE AND share_with_ai = TRUE')
    ])
    ...
    docsRes.rows.slice(0, 10).forEach(d => {
      const safeText = d.extracted_text ? d.extracted_text.substring(0, 2000) + (d.extracted_text.length > 2000 ? '... [truncated]' : '') : 'No text available'
      dynamicContext += `- ${d.title}:\n  ${safeText}\n`
    })
    ...
    const messages = [
      ...
      // Keep only the last 6 messages (3 turns) to prevent context from ballooning over time
    ...
    // Handle Groq rate limits or massive payloads gracefully
    if (err.status === 413 || err.status === 429 || err.message.includes('rate_limit_exceeded')) {
      return res.status(429).json({ message: "My brain is a bit full right now! Please wait a moment and try asking again." })
    }
...
router.post('/notice', protect, authorize('admin'), async (req, res) => {
  try {
...
router.post('/remark', protect, authorize('teacher', 'admin'), async (req, res) => {
  try {
